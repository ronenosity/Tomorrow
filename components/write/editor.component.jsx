...
const CREATE_THREAD_MUTATION = gql`
  mutation CREATE_THREAD_MUTATION($community: String!, $title: String!, $content: String!, $date: String!) {
    createThread(slug: $community, title: $title, content: $content, date: $date) {
      id
      slug
      community {
        id
        ...
        <Header>
          <Query query={COMMUNITY_NAME_QUERY} variables={{ community: this.props.community }}>
            {({ loading, error, data: { community } }) => {
              ...
              return (
                <Title>
                  New thread on <span>{get(community, 'name')}</span> community
                </Title>
              );
            }}
          </Query>
