...
const ThreadPreview = ({ thread, refetch }) => {
  return (
    <QueryStringConsumer>
      {queryString => {
        const activeThread = queryString.t === thread.slug;
        return (
          <>
            {activeThread && (
              <Head>
                <title>RFS | {thread.title}</title>
              </Head>
            )}
            <Link route="dashboard" params={{ c: thread.community.slug, t: thread.slug }}>
              <Thread active={activeThread}>
                <AuthorPicture picture={thread.author.avatar} />
                <ThreadDetails>
                  <ThreadTitle>{moment(thread.date).format('MM/DD/YYYY')}</ThreadTitle>
                  <ThreadTitle dangerouslySetInnerHTML={{ __html: md.render(thread.title) }} />
                  <ThreadMeta>
                    <ThreadInfo>
                      <ThreadAuthor>{`${get(thread, 'author.name')} ${
                        thread.author.lastName
                      }`}</ThreadAuthor>
                      <span>–</span>
                      <ThreadDate>{distanceInWordsStrict(thread.createdAt, Date.now())}</ThreadDate>
                    </ThreadInfo>
                  </ThreadMeta>
                  <ThreadStats>
                    <ThreadStat data-tooltip="Replies on this thread">
                      <RepliesIcon fill={scheme.gray[7]} />
                      <ThreadStatValue>{thread.replies_count}</ThreadStatValue>
                    </ThreadStat>
                    <ThreadStat data-tooltip="Members on this thread">
                      <UsersIcon fill={scheme.gray[7]} />
                      <ThreadStatValue>{thread.users_replying}</ThreadStatValue>
                    </ThreadStat>
                    <Likes thread={thread} refetch={refetch} />
                  </ThreadStats>
                </ThreadDetails>
              </Thread>
            </Link>
