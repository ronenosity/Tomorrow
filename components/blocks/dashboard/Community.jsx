...
const Community = ({ community }) => (
  <QueryStringConsumer>
    {queryString => {
      const activeCommunity = queryString.c === community.slug;
      return (
        <>
          {activeCommunity && (
            <Head>
              <title>RFS |{get(community, 'name')}</title>
            </Head>
          )}
          <Link route="dashboard" params={{ c: community.slug }}>
            <Container active={activeCommunity}>
              <CommunityPicture picture={community.picture} />
              <CommunityDetails>
                <CommunityName>{get(community, 'name')}</CommunityName>
